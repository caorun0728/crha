		...
		case $num1 in
			...
			1)
				...
				sleep 3
					...
					read -s -n 1 -p "是否刷入SeryoVPN加速插件？请输入[Y/N]?" yn
					if [ "$yn" == "Y" ] || [ "$yn" == "y" ]; then
					echo ""
					echo "开始配置配置文件"
					...
					echo ""
					echo -e "是否检查是否有数据更新？[Y/N]" updateatinstall
					if [ "$updateatinstall" == "Y" ] || [ "$updateatinstall" == "y" ]; then
							...
							echo "开始检查是否有更新"
							sleep 3
							if [ -f /usr/bin/vendor/config/ver2016-03-17 ]; then
									echo "当前已经是最新版本，无需更新"
						
								else
									echo "发现新版本文件，开始更新！"
									sleep 2
									wget -O /usr/bin/vendor/config/ver2016-03-17 ${URL}/update/version_check
									wget -O /tmp/update.tar.gz ${URL}/update/update.tar.gz
									tar xvzf /tmp/update.tar.gz
									rm -f /usr/bin/vendor/config/gfw_list.conf
									cp -r -a /tmp/update/* /
									echo -e "更新rixCloud 云加速插件-----------------[\e[32m完成\e[37m]"
									echo "本次更新的内容有："
									echo "更新了老版本的GFWList，当前版本：3938条规则"
									sleep 3
								fi
